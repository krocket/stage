...
class SaleOrderInherit(models.Model):
    ...
    def _cart_update_t_shirt(self, product_id=None, line_id=None, add_qty=0, set_qty=0, received_combination=None,
        ...
        product_with_context = self.env['product.template'].with_context(product_context)
        product = product_with_context.search([('id', '=', product_id)])
        ...
        if not order_line:
            ...
            combination = product._get_closest_possible_combination(received_combination)

            # get or create (if dynamic) the correct variant
            product = product._create_product_variant(combination)
